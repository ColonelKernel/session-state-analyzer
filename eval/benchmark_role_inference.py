...
Runs :func:`session_explorer.core.roles.infer_role` over
...
Usage:
    python3 eval/benchmark_role_inference.py \
...
from session_explorer.core import roles as role_inference  # noqa: E402
...
def main(argv: list[str]) -> int:
    sources = argv or [
        os.path.join(_ROOT, "data", "eval", "medleydb_instruments.csv"),
        os.path.join(_ROOT, "data", "eval", "curated_filenames.csv"),
    ]
    rows = load_rows(sources)
    results = evaluate(rows)
    os.makedirs(os.path.join(_ROOT, "data", "eval"), exist_ok=True)
    with open(os.path.join(_ROOT, "data", "eval", "results.json"), "w", encoding="utf-8") as fh:
        json.dump(results, fh, indent=2)
    os.makedirs(os.path.join(_ROOT, "docs"), exist_ok=True)
    with open(os.path.join(_ROOT, "docs", "evaluation.md"), "w", encoding="utf-8") as fh:
