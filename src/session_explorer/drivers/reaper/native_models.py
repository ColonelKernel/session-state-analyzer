"""Typed native data models for parsed REAPER DAW-state (verbatim prototype models).
...
The ``ProjectState`` family below is byte-faithful to the REAPER prototype's
``models.py``; the prototype's ``AudioDescriptorSet`` and ``Recommendation``
duplicates were dropped because the canonical core models cover them and no
module in this package references the native versions.
"""
