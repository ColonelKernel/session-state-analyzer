"""The REAPER dialect rule pack: guide-cited, explainable heuristics.

The eleven rules are ported verbatim from the REAPER prototype's
``recommendations.py`` — same thresholds, same wording, same page citations
into the two official guides. Each rule is exposed as a
:class:`~session_explorer.core.driver.Rule` whose function takes a
:class:`~session_explorer.core.models.CanonicalSession`: internally it
reconstructs the verbatim native :class:`ProjectState` via
:func:`.mapper.to_native`, runs the unchanged prototype logic, and then
namespaces the native node ids in ``related_node_ids`` into canonical id
space (``reaper:track-0``). Descriptor-derived node ids (rules 5 and 10) are
already canonical-space ids and pass through untouched.
...
from ...core.models import AudioDescriptorSet, CanonicalSession, Recommendation
from .fx_knowledge import STOCK_FX, WORKFLOWS, lookup_stock_fx
from .keywords import is_ambience_fx, is_vocal_name
from .mapper import DIALECT, to_native
from .native_models import ProjectState, TrackState
...
# ---------------------------------------------------------------------------
# Canonical Rule wrappers
# ---------------------------------------------------------------------------

_ProjectRuleFn = Callable[[ProjectState], List[Recommendation]]
_DescriptorRuleFn = Callable[[List[AudioDescriptorSet]], List[Recommendation]]


def _project_rule(fn: _ProjectRuleFn) -> Callable[[CanonicalSession], List[Recommendation]]:
    """Run verbatim native-model logic, then namespace the native node ids."""

    def wrapped(session: CanonicalSession) -> List[Recommendation]:
        project = to_native(session)
        recommendations = fn(project)
        for rec in recommendations:
            rec.related_node_ids = [
                namespaced(DIALECT, node_id) for node_id in rec.related_node_ids
            ]
...
def _descriptor_rule(fn: _DescriptorRuleFn) -> Callable[[CanonicalSession], List[Recommendation]]:
    """Descriptor rules read canonical descriptors, whose node ids are already
    canonical-space — they pass through un-namespaced."""

    def wrapped(session: CanonicalSession) -> List[Recommendation]:
        return fn(list(session.descriptors))

    wrapped.__name__ = fn.__name__
    return wrapped


REAPER_RULES: List[Rule] = [
    Rule(
        rule_id="reaper.shared_ambience_bus",
        fn=_project_rule(_rule_shared_ambience_bus),
        requires=["plugin_chain", "bus_routing"],
        description="Individual ambience FX with no shared send/return",
    ),
    Rule(
        rule_id="reaper.vocal_chain",
        fn=_project_rule(_rule_vocal_chain),
        requires=["plugin_chain"],
        description="Vocal track without a basic vocal chain",
    ),
    Rule(
        rule_id="reaper.dense_fx",
        fn=_project_rule(_rule_dense_fx),
        requires=["plugin_chain"],
        description="Dense FX chain (> 6 processors on one track)",
    ),
    Rule(
        rule_id="reaper.missing_bus",
        fn=_project_rule(_rule_missing_bus),
        requires=["bus_routing"],
        description="Many tracks but no routes/sends detected",
    ),
    Rule(
        rule_id="reaper.level_imbalance",
        fn=_descriptor_rule(_rule_level_imbalance),
        requires=[],
        description="Descriptor-based level imbalance (stems much hotter than the rest)",
    ),
    Rule(
        rule_id="reaper.all_fx_offline",
        fn=_project_rule(_rule_all_fx_offline),
        requires=["plugin_chain"],
        description="Every FX offline (recovery-mode fingerprint)",
    ),
    Rule(
        rule_id="reaper.muted_sends",
        fn=_project_rule(_rule_muted_sends),
        requires=["sends"],
        description="Muted or near-silent sends (routing debris)",
    ),
    Rule(
        rule_id="reaper.bypassed_fx",
        fn=_project_rule(_rule_bypassed_fx),
        requires=["plugin_chain"],
        description="Bypassed-but-online FX accumulating on a track",
    ),
    Rule(
        rule_id="reaper.manual_submix",
        fn=_project_rule(_rule_manual_submix),
        requires=["bus_routing", "mixer_state"],
        description="Manual submix where the guide prefers a folder track",
    ),
    Rule(
        rule_id="reaper.clipping_risk",
        fn=_descriptor_rule(_rule_clipping_risk),
        requires=[],
        description="Clipping-risk stems (peak at or near digital full scale)",
    ),
    Rule(
        rule_id="reaper.meters_in_render_path",
        fn=_project_rule(_rule_meters_in_render_path),
        requires=["plugin_chain"],
        description="Analysis/metering FX in the render path",
    ),
]
