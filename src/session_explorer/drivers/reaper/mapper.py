"""Lossless mapping between the native REAPER ``ProjectState`` and the canonical schema.

Losslessness contract (the Phase 2 gate):

    ``to_native(to_canonical(project)).model_dump() == project.model_dump()``

The complete native model rides along as ``session.native`` (a
:class:`~session_explorer.core.models.NativePayload`), so nothing the parser
