...
Ported verbatim from the Logic prototype (``logic_session_evidence_explorer.
utils``). The native id vocabulary (``audio_1``, ``track_2``) is kept distinct
from :mod:`session_explorer.core.ids` (``track-1``) deliberately: native ids
live inside the verbatim native payload, canonical ids are namespaced by the
mapper (``logic:audio_1``).
"""
