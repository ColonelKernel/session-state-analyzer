...
trained classifiers). It can be non-trivial to install on some platforms, so the
product never depends on it. When Essentia is importable, ``maybe_extract_highlevel``
...
def maybe_extract_highlevel(path: str) -> Dict[str, float]:
    ...
    try:  # pragma: no cover - exercised only where Essentia is installed
        ...
        try:
            # Frame-wise spectral complexity (number of spectral peaks), the
            # descriptor the Ableton prototype surfaced as a mix-density cue.
            ...
            if values:
                extra["essentia_spectral_complexity_mean"] = float(
