...
def reconcile_stem_sum(
    ...
    *choice*, not a default (Range and Include Audio Tail options; see the
    dialect's export instructions). Duration mismatches are warned about and
    truncated to the shortest signal.
    """

    # Alignment is an export-settings assumption, not a DAW default: e.g.
    # Logic's track-export dialog's Range option can trim silence at file end
    # or export only the cycle range, and Include Audio Tail extends files
...
def compare_to_reference(
    ...
    *,
    ...
    try:
        ...
        result.crest_delta_db = crest if crest is not None else _delta("dynamic_range_db")
