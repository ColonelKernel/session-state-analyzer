"""Tests for the REAPER rule pack, run through the canonical rule engine.

Ported from the prototype's ``test_recommendations.py``: fixtures are built by
parsing the same ``.rpp`` snippets, wrapped through ``mapper.to_canonical``,
and evaluated with ``core.recommend.run_rules``. Assertions on rule ids,
titles, severities, and citation presence are preserved (golden parity);
track/FX/route node ids are now namespaced (``reaper:...``), while descriptor
node ids stay canonical-space and un-prefixed.
"""
...
from session_explorer.drivers.reaper.rules import REAPER_RULES
...
def _recs_for(rpp: str, descriptors=None):
    session = to_canonical(parse_rpp(rpp))
    if descriptors:
        session.descriptors = list(descriptors)
    return run_rules(session, REAPER_RULES)
...
def test_level_imbalance_recommendation():
    ...
    # Descriptor node ids are canonical-space already: no reaper: prefix.
...
"""
    ...
    # Route node ids are namespaced into canonical id space.
    assert all(node_id.startswith("reaper:") for node_id in cards[0].related_node_ids)
...
def test_no_rules_are_skipped_for_rpp_evidence():
    # The rpp_file artifact hides plugin_parameters/automation/track_stack only;
    # none of the eleven REAPER rules require those, so the engine must never
    # emit "not evaluable" info cards for a parsed .rpp session.
    recs = _recs_for('<REAPER_PROJECT 0.1 "x" 0\n  <TRACK\n    NAME "Lead Vox"\n  >\n>\n')
    assert not any("not evaluable" in r.title for r in recs)
